...
from dataset_forge.bhi_filtering import BlockinessThread, HyperThread, IC9600Thread
...
def render_report(stats, plot_path, sample_paths, output_path, format='html',
                  quality_hist_paths=None, lq_quality_hist_paths=None,
                  class_balance_path=None, lq_class_balance_path=None):
    env = Environment(loader=FileSystemLoader(os.path.join(os.path.dirname(__file__), '../../reports/templates')))
    ...
    report = template.render(
        ...
        sample_paths=[os.path.relpath(p, os.path.dirname(output_path)) for p in sample_paths],
...
def generate_rich_report(hq_path=None, lq_path=None, single_folder_path=None, output_path=None, format="html", sample_count=5, max_quality_images=100):
    """
    ...
    if single_folder_path:
        files = [os.path.join(single_folder_path, f) for f in os.listdir(single_folder_path) if is_image_file(f)]
        report_name = os.path.basename(single_folder_path.rstrip('/\\'))
    elif hq_path and lq_path:
        files = [os.path.join(hq_path, f) for f in os.listdir(hq_path) if is_image_file(f)]
        report_name = f"{os.path.basename(hq_path.rstrip('/\\'))}_HQ"
    else:
    ...
    # Samples
    sample_paths = save_sample_images(stats['filepaths'], output_dir, sample_count=sample_count)
    # Quality scores and plots
    quality_scores = compute_quality_scores(single_folder_path or hq_path, max_images=max_quality_images)
    quality_hist_paths = plot_quality_histograms(quality_scores, output_dir, prefix="HQ_" if hq_path else "")
    if hq_path and lq_path:
        lq_quality_scores = compute_quality_scores(lq_path, max_images=max_quality_images)
        lq_quality_hist_paths = plot_quality_histograms(lq_quality_scores, output_dir, prefix="LQ_")
    else:
    ...
    if class_counts:
        class_balance_path = plot_class_balance(class_counts, output_dir, prefix="HQ_" if hq_path else "")
    else:
        class_balance_path = None
    if hq_path and lq_path:
        lq_class_counts = compute_class_balance(lq_path)
        if lq_class_counts:
            lq_class_balance_path = plot_class_balance(lq_class_counts, output_dir, prefix="LQ_")
        else:
    ...
    # Render
    render_report(
        stats, plot_path, sample_paths, output_path, format=format,
        quality_hist_paths=quality_hist_paths, lq_quality_hist_paths=lq_quality_hist_paths,
        class_balance_path=class_balance_path, lq_class_balance_path=lq_class_balance_path
    )
