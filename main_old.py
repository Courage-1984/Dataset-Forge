...
def main_menu():
    ...
    def tiling_menu():
        ...
        if tiling_type == "best":
            ...
            if mode == "1":
                ...
                n_tiles = input(
                    "Number of tiles per image (blank for default): "
                ).strip()
    ...
    def de_dupe_menu():
        ...
        if mode == "2":
            groups = (
                de_dupe.find_near_duplicates(hq_hashes, max_distance=max_dist) or []
            )
        ...
        dest_dir = None
        if op in ("move", "copy"):
            hq_dupes = os.path.join(
                os.path.dirname(hq), os.path.basename(hq) + "_dupes"
            )
            lq_dupes = os.path.join(
                os.path.dirname(lq), os.path.basename(lq) + "_dupes"
            )
    ...
    def batch_rename_menu():
        ...
        try:
            ...
            padding = 5
        dry_run = (
            input("Dry run? (show what would be renamed, y/n, default y): ")
            .strip()
            .lower()
        )
        dry_run = dry_run != "n"
    ...
    def hue_adjustment_menu():
        ...
        input_path = input(
            "Enter input folder path (single or parent of hq/lq): "
        ).strip()
        if not os.path.exists(input_path):
            print(f"Input path does not exist: {input_path}")
            return
        ...
        try:
            ...
            duplicates = 1
        enable_brightness = (
            input("Enable brightness shifting? (y/n, default n): ").strip().lower()
            == "y"
        )
        enable_contrast = (
        ...
        enable_hue = (
            input("Enable hue shifting? (y/n, default n): ").strip().lower() == "y"
        )
        brightness = None
        contrast = None
        hue = None
        ...
        if enable_contrast:
            try:
                contrast = float(
                    input("Base contrast factor (e.g. 1.0): ").strip() or 1.0
                )
            except Exception:
                contrast = 1.0
        if enable_hue:
    ...
    def compare_folders_menu():
        ...
        missing1, missing2 = folder_compare.compare_folders(
    ...
    def move_copy_menu():
        ...
        if is_pair:
            ...
            print(f"Detected single folder: {input_path}")
        op = (
            input("Choose operation: (m)ove or (c)opy [m/c, default m]: ")
            .strip()
            .lower()
        )
        operation = "copy" if op == "c" else "move"
        ext = input(
            "Move/copy only files with this extension (e.g. .png), blank for all: "
        ).strip()
        ext = ext if ext else None
        percent = input(
            "Percentage of files/pairs to move/copy (e.g. 50 for half, default 100): "
        ).strip()
        try:
            percent = float(percent) if percent else 100.0
        except Exception:
            percent = 100.0
        percent = int(percent)  # Ensure int for move_percentage
        output_path = input("Enter output folder path: ").strip()
        if not output_path:
            print("Output folder path required.")
            return
        os.makedirs(output_path, exist_ok=True)
        seed = input("Random seed for reproducibility (blank for random): ").strip()
        seed = int(seed) if seed else None
        if is_pair:
            move_copy.move_or_copy_hq_lq_pairs(
                input_folder=input_path,
                output_folder=output_path,
                move_percentage=percent,
                file_extension=ext,
                operation=operation,
                seed=seed,
            )
        else:
            move_copy.move_or_copy_files(
        ...
        print("Move/Copy operation complete.")

    def find_misaligned_images_menu():
        print("\n=== Find Misaligned Images (HQ/LQ Pair) ===")
        input_path = input(
            "Enter input folder path (should contain 'hq' and 'lq' subfolders): "
        ).strip()
        hq_path = os.path.join(input_path, "hq")
        lq_path = os.path.join(input_path, "lq")
        if not (os.path.isdir(hq_path) and os.path.isdir(lq_path)):
            print_error("Input folder must contain both 'hq' and 'lq' subfolders.")
            return
        output_path = input("Enter output folder for misaligned pairs: ").strip()
        if not output_path:
            print_error("Output folder path required.")
            return
        os.makedirs(output_path, exist_ok=True)
        op = (
            input("Move or copy misaligned pairs? (m=move, c=copy, default m): ")
            .strip()
            .lower()
        )
        operation = "copy" if op == "c" else "move"
        try:
            threshold = float(
                input("Similarity threshold (default 0.7, lower = stricter): ").strip()
                or 0.7
            )
        except Exception:
            threshold = 0.7
        try:
            num_workers = int(input("Number of workers (default 8): ").strip() or 8)
        except Exception:
            num_workers = 8
        from dataset_forge.misalignment import MisalignmentDetector

        detector = MisalignmentDetector(threshold=threshold, num_workers=num_workers)
        print_info(
            f"Scanning for misaligned pairs (threshold={threshold}, operation={operation})..."
        )
        moved = detector.scan_and_process(hq_path, lq_path, output_path, op=operation)
        print_success(
            f"Done! {len(moved)} misaligned pairs {'moved' if operation=='move' else 'copied'} to {output_path}."
        )
        if moved:
            print_info("Sample of misaligned files:")
            for f in moved[:10]:
                print(f"  {f}")
    ...
    def orientation_organization_menu():
        ...
        if is_pair:
            ...
            print(
                f"Done! Processed images for orientations: {', '.join(orientations)}."
            )
    ...
    def bhi_filtering_menu():
        ...
        try:
            hyperiqa_thr = float(
                input("HyperIQA threshold (default 0.5): ").strip() or 0.5
            )
        except Exception:
            hyperiqa_thr = 0.5
        try:
    ...
    option_map = {
        ...
        "23": ("Image Tiling (Best/Linear/Random/Overlap, Single/Pair)", tiling_menu),
        "24": (
        ...
        "29": ("De-Dupe: Find & Handle Duplicate/Near-Duplicate Images", de_dupe_menu),
        "30": (
            "ICC to sRGB Conversion (Preserve Alpha, Batch/Folder Support)",
            icc_to_srgb_menu,
        ),
        "31": ("Metadata (EXIF) Scrubber", exif_scrubber_menu),
        "32": ("Batch Renaming Utility (Single/Paired HQ-LQ)", batch_rename_menu),
        "33": (
            "Hue/Brightness/Contrast Adjustment (Single/Paired HQ-LQ, Duplicates)",
            hue_adjustment_menu,
        ),
        "34": ("Compare Folders (Show Missing Files)", compare_folders_menu),
        "35": (
            "Move/Copy Files (Single or Paired HQ-LQ, %/ext support)",
            move_copy_menu,
        ),
        "36": ("Find Misaligned Images (HQ/LQ Pair)", find_misaligned_images_menu),
        "37": (
            "Images Orientation Organization (Extract by Landscape/Portrait/Square)",
            orientation_organization_menu,
        ),
        "38": ("BHI FILTERING (Blockiness, HyperIQA, IC9600)", bhi_filtering_menu),
        "39": ("Multiscale Dataset (DPID Downscale + Tiling)", multiscale_dataset_menu),
    }
