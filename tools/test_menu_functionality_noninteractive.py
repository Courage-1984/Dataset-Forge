#!/usr/bin/env python3
"""
Non-interactive test runner for menu functionality after lazy implementation.

This script provides a simple way to test that all menus work correctly
after the lazy import implementation without requiring user input.
"""
...
def test_menu_navigation_sequence():
    """Test menu navigation sequence with lazy loading."""
    print("\n🔍 Testing menu navigation sequence...")
    
    try:
        from dataset_forge.menus.main_menu import main_menu
        
        # Mock show_menu to simulate navigation through multiple levels
        navigation_sequence = ['1', '1', '0', '0']  # Main -> Dataset Management -> Create Dataset -> Back -> Back
        
        with patch('dataset_forge.utils.menu.show_menu', side_effect=navigation_sequence):
            with patch('dataset_forge.utils.audio_utils.play_startup_sound'):
                main_menu()
        
        print("  ✅ Menu navigation sequence works")
        return True
    except Exception as e:
        print(f"  ❌ Menu navigation sequence: {e}")
        return False

def test_lazy_import_consistency():
    """Test that lazy imports are consistent across calls."""
    print("\n🔍 Testing lazy import consistency...")
    ...
    try:
        ...
        if torch1 is torch2:
            print("  ✅ Lazy import consistency confirmed")
            return True
        else:
            print("  ❌ Lazy import consistency failed")
            return False
    except Exception as e:
        print(f"  ❌ Lazy import consistency: {e}")
        return False

def main():
    """Run all tests."""
    print("🚀 Testing Menu Functionality After Lazy Implementation (Non-Interactive)")
    print("=" * 70)
    ...
    tests = [
        ...
        ("Menu Navigation Sequence", test_menu_navigation_sequence),
        ("Lazy Import Consistency", test_lazy_import_consistency),
    ]
    ...
    if passed == len(results):
        ...
        print("✅ Lazy import implementation is successful!")
        return 0
    else:
