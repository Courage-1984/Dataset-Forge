...
class MochaThemingChecker:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.centralized_printing_utils = {
            ...
            'print_header', 'print_section', 'print_subsection', 'print_note',
            'print_debug', 'print_verbose', 'print_progress', 'print_status'
        }
    ...
    def should_analyze_file(self, file_path: Path) -> bool:
        ...
        if not any(file_path.is_relative_to(self.project_root / dir_name) 
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if 'mocha' in alias.name.lower() or 'color' in alias.name.lower():
                            has_mocha_import = True
                elif isinstance(node, ast.ImportFrom):
                    if 'dataset_forge.utils.color' in getattr(node.module, 'value', ''):
                        has_mocha_import = True
                    elif 'dataset_forge.utils.printing' in getattr(node.module, 'value', ''):
                        has_printing_imports = True
        except SyntaxError:
            issues.append(ThemingIssue(
                file_path=str(file_path),
                line_number=0,
    ...
    def _analyze_menu_implementation(self, file_path: Path, content: str, lines: List[str], issues: List[ThemingIssue]):
    ...
    def run_analysis(self) -> Dict[str, Any]:
        ...
        print("🔍 Starting Catppuccin Mocha Theming Consistency Analysis...")
    ...
    def generate_report(self, summary: Dict[str, Any], output_file: Optional[str] = None) -> str:
        ...
        report_lines.append(f"- **Raw Print Statements:** {summary['total_raw_prints']}")
        report_lines.append(f"- **Centralized Print Usage:** {summary['total_centralized_prints']}")
        report_lines.append("")
        ...
        for file_path, issues in sorted(issues_by_file.items()):
            ...
            for issue in sorted(issues, key=lambda x: x.line_number):
                ...
                report_lines.append(f"**Line {issue.line_number}** {severity_icon} **{issue.issue_type.replace('_', ' ').title()}**")
        ...
        if summary['total_raw_prints'] > 0:
            ...
            report_lines.append("Replace all raw `print()` statements with centralized printing utilities:")
            report_lines.append("")
            ...
            report_lines.append("from dataset_forge.utils.printing import print_success")
        ...
        if summary['issues_by_type'].get('missing_menu_context', 0) > 0:
            ...
            report_lines.append("Add missing `current_menu` and `menu_context` parameters to `show_menu` calls:")
            report_lines.append("")
            report_lines.append("```python")
        ...
        report_lines.append("")
        report_lines.append("1. **Always use centralized printing utilities** instead of raw `print()`")
        ...
        report_lines.append("4. **Follow the Catppuccin Mocha color scheme** consistently")
...
Examples:
  ...
  python tools/check_mocha_theming.py --verbose
    ...
    print("\n" + "="*60)
    print("🎨 CATPPUCCIN MOCHA THEMING ANALYSIS COMPLETE")
    print("="*60)
    print(f"📁 Files Analyzed: {summary['total_files']}")
    print(f"🚨 Total Issues: {summary['total_issues']}")
    print(f"🔴 Errors: {summary['issues_by_severity'].get('error', 0)}")
    print(f"🟡 Warnings: {summary['issues_by_severity'].get('warning', 0)}")
    print(f"🔵 Info: {summary['issues_by_severity'].get('info', 0)}")
    print(f"📄 Raw Prints: {summary['total_raw_prints']}")
    print(f"✅ Centralized Prints: {summary['total_centralized_prints']}")
    print("="*60)
    ...
    if summary['issues_by_severity'].get('error', 0) > 0:
        ...
        print("\n⚠️  Theming issues found. Review the report for details.")
        sys.exit(0)
    else:
